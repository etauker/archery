...
class GlucosePersistenceManager {
     ...
     */
    public getTransaction = function(oTransactionFilter: GlucoseTransaction, sUserId: string) {
        if (!sUserId) throw this.error.getError(1, sUserId, '', 'Missing user id.');
        if (!oTransactionFilter) throw this.error.getError(1, oTransactionFilter, '', 'Missing transaction object.');
     ...
     *  @return {promise} Resolves to the transaction entry from the database.
     */
    public getTransactionById = function(sTransactionId: string, sUserId: string) {
        if (!sTransactionId) throw this.error.getError(1, sTransactionId, '', 'Missing transaction id.');
        if (!sUserId) throw this.error.getError(1, sUserId, '', 'Missing user id.');
    ...
    public getTransactions = function(oTransactionFilter: GlucoseTransaction, sUserId: string) {
        if (!sUserId) throw this.error.getError(1, sUserId, '', 'Missing user id.');
        if (!oTransactionFilter) oTransactionFilter = new GlucoseTransactionInstance();
    ...
    public saveTransaction = function(oTransactionObject: GlucoseTransaction, sUserId: string) {
        if (!sUserId) throw this.error.getError(1, sUserId, '', 'Missing user id.');
        if (!oTransactionObject) throw this.error.getError(1, oTransactionObject, '', 'Missing transaction object.');
    ...
    //             PRIVATE FUNCTIONS
    //===========================================
    private checkExpectedCount = function(iExpectedCount, aResults) {

...
class GlucoseReadingsServiceValidator {
    //===========================================
    //               CONSTRUCTOR
    //===========================================
    constructor(paths) {
        ...
        this.validateTransaction = (oTransaction) => {
            this.logger.logObject(oTransaction, 'start', 'validateTransaction', 'GlucoseReadingsServiceValidator');
            if (!oTransaction) {
                this.lastError = this.error.getError(2);
                return null;
            }
            // oObject, sProperty, sDatatype, bOptional, bNullable
            if (!this._checkProperty(oTransaction, "id", "string", true, true))
                return null;
            if (!this._checkProperty(oTransaction, "dateTime", "number", true, true))
                return null;
            if (!this._checkProperty(oTransaction, "reading", "number", true, true))
                return null;
            if (!this._checkProperty(oTransaction, "carbohydrates", "number", true, true))
                return null;
            if (!this._checkProperty(oTransaction, "insulinUnitsShort", "number", true, true))
                return null;
            if (!this._checkProperty(oTransaction, "insulinUnitsLong", "number", true, true))
                return null;
            if (!this._checkProperty(oTransaction, "correctionUnits", "number", true, true))
                return null;
            if (!this._checkProperty(oTransaction, "meal", "string", true, true))
                return null;
            if (!this._checkProperty(oTransaction, "note", "string", true, true))
                return null;
            try {
            ...
            this.logger.logObject(oTransaction, 'end', 'validateTransaction', 'GlucoseReadingsServiceValidator');
            return oTransaction;
        };
        ...
        //             PRIVATE FUNCTIONS
        //===========================================
        this._checkProperty = function (oObject, sProperty, sDatatype, bOptional, bNullable) {
            // Property is optional and doesn't exist - true
            if (bOptional === true && !oObject[sProperty])
                return true;
            ...
            if (bNullable === true && oObject[sProperty] === null)
                return true;
            ...
            if (typeof oObject[sProperty] === sDatatype)
                return true;
        ...
        this.error = new GlucoseErrorGenerator("com.etauker.glucose", "utils", "GlucoseReadingsServiceValidator", this.errorMessages);
        this.lastError = null;
    }
}
;
module.exports = GlucoseReadingsServiceValidator;

...
class SecurityTokenManager {
    constructor(oPersistenceManager) {
        ...
        this.error = new SecurityErrorGenerator("com.etauker.security", "logic", "SecurityTokenManager", [
...
SecurityTokenManager.prototype.verifyToken = function (sToken, sRole) {
    return new Promise((fnResolve, fnReject) => __awaiter(this, void 0, void 0, function* () {
        // Local variables
        let oToken;
        const oOptions = {
        ...
        // Verify token
        try {
            oToken = yield jwt.verify(sToken, process.env.JWT_SECRET, oOptions);
            // console.log("=== oToken ===");
            // console.log(oToken);
            // Check the if the session has been invalidated in the database
            const bInvalidated = yield this.persistenceManager.checkSessionValidity(oToken.id)
                .then(bValid => !bValid)
                .catch(oError => fnReject(oError));
            if (bInvalidated) {
                fnReject(this.error.getError(4));
            }
            else {
                ...
                if (sRole) {
                    ...
                    if (!bHasRole) {
                        fnReject(this.error.getError(5));
                    }
                }
                fnResolve(oToken);
            }
            ...
            if (oError.name === "TokenExpiredError") {
                fnReject(this.error.getError(2, oError));
            }
            if (oError.name === "JsonWebTokenError") {
